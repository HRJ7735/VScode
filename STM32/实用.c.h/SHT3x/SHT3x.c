...
//IIC总线接口定义
#define SCL PBout(4)//设置时钟端口
//SHT30数据SDA传输口，有输出也有输入，所以需配置它的输入和输出
#define SDA_OUT PBout(3)//设置输出数据端口
#define SDA_IN PBin(3)//设置输入数据端口
//设置端口高8位的工作模式（具体可以参照端口位配置表），即I/O输入输出方向设置，先用与（&）对PB7引脚的四个位清0，再用或（|）置1
#define IIC_INPUT_MODE_SET()  {GPIOB->CRL&=0XFFFF0FFF;GPIOB->CRL|=8<<12;}//改变PB3对应位为1000（CNF[1:0]MODE[1:0]），设置成上拉或者下拉输入
#define IIC_OUTPUT_MODE_SET() {GPIOB->CRL&=0XFFFF0FFF;GPIOB->CRL|=3<<12;}//改变PB3对应位为0011，设置成通用推挽输出
...
//  __HAL_RCC_GPIOA_CLK_ENABLE();
  __HAL_RCC_GPIOB_CLK_ENABLE();
	...
	GPIO_InitStruct.Pin = GPIO_PIN_3|GPIO_PIN_4;//定于SCL连接到STM32的PB6引脚上，定于SDA_OUT/SDA_IN连接到STM32的PB7引脚上
...
//主机等待从机的确认字符ACK
uint8_t IIC_wait_ACK(void)//主机等待从机发送ACK，从而判断数据是否接收完成
{
    uint8_t t = 200;
    IIC_OUTPUT_MODE_SET();
