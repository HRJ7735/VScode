...
//(x,y):中心点
//r    :半径
void Draw_Circle(uint16_t x0,uint16_t y0,uint8_t r, uint16_t RGB_Code)
{
	int a,b;
	int di;
	a=0;b=r;	  
...
void LCD_ShowChar(uint16_t x,uint16_t y,uint8_t num,uint8_t size, uint16_t RGB_Code ,uint16_t back_color)
{  							  
    uint8_t temp,t1,t;
	uint16_t y0=y;		   
	num=num-' ';//得到偏移后的值
 for(t=0;t<size;t++)
	{   
	        ...
	        for(t1=0;t1<8;t1++)
					{			    
		        if(temp&0x80)LCD_DrawPoint(x,y,RGB_Code);	
...
//color:颜色 
//num:数值(0~4294967295);	 
void LCD_ShowNum(uint16_t x,uint16_t y,uint32_t num,uint8_t len,uint8_t size ,uint16_t RGB_Code,uint16_t back_color)
{         	
	uint8_t t,temp;
	uint8_t enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_ShowChar(x+(size/2)*t,y,' ',size ,RGB_Code,back_color);
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar(x+(size/2)*t,y,temp+'0',size, RGB_Code,back_color); 
	}
} 
//显示数字,高位为0,还是显示
//x,y:起点坐标
...
//[6:1]:保留
//[0]:0,非叠加显示;1,叠加显示.
void LCD_ShowxNum(uint16_t x,uint16_t y,uint32_t num,uint8_t len,uint8_t size, uint16_t RGB_Code,uint16_t back_color)
{  
	uint8_t t,temp;
	uint8_t enshow=0;						   
	for(t=0;t<len;t++)
...
uint16_t findHzIndex(uint8_t *hz)                     /* 在自定义汉字库在查找所要显示 */                                                     /* 的汉字的位置 */
{
    uint32_t i=0;
...
void WriteOneHzChar(uint16_t x,uint16_t y,uint8_t *font ,  uint16_t RGB_Code,uint16_t back_color)
{
	uint8_t temp,i,j;
	uint16_t y0=y;  
	uint8_t *ch;     
	
	ch = GBHZ_16[findHzIndex(font)].Msk ;
	    ...
	    {   												   
	        ...
	        for(j=0;j<8;j++)
			 {
					if(temp&0x80)LCD_DrawPoint(y,x ,RGB_Code);
					else LCD_DrawPoint(y,x ,back_color);
					temp<<=1;
					y++;
					if((y-y0)==16)
					{
...
void LCD_ShowString(uint16_t x0, uint16_t y0,uint8_t *pcStr,  uint16_t RGB_Code,uint16_t back_color)
{
	...
	while(1)
  {
        if(*pcStr == 0) break; /* 字符串结束            */     
        x0 = x0 + (usWidth);                           /* 调节字符串显示松紧度         */
    if(*pcStr > 0x80)                              /* 判断为汉字                   */
     {
		    if((x0 + 16) > 240)                      /* 检查剩余空间是否足够         */
            {
			    x0 = 0;
                y0 = y0 + 16;                          /* 改变显示坐标                 */
                if(y0 > 320)                         /* 纵坐标超出                   */
            ...
            WriteOneHzChar(y0, x0,  pcStr , RGB_Code, back_color);
            x0+=16;                                           /* 显示字符                     */
            pcStr += 2;
      }
		else 
    ...
    while((*pcStr<='~')&&(*pcStr>=' '))//判断是不是非法字符!
    {       
      
        LCD_ShowChar(x0,y0,*pcStr,16,  RGB_Code,back_color);
        x0+=8;
        pcStr++;
    }  
		}
